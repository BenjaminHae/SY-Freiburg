...
class syDB{
   ...
   {
		settype($gp, 'integer');
		$sql = "SELECT * FROM scotland_yard WHERE scotland_yard.group = $gp ORDER BY timestamp DESC LIMIT 1;";
   ...
   public function GetGroupMovement($gp, $limit=0)
   {
		...
		$sql = "SELECT * FROM scotland_yard WHERE scotland_yard.group = $gp ORDER BY timestamp DESC$limString;";
		$query = mysql_query($sql, $this->connection);
